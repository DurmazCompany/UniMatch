...
export function MatchModal({ match, currentUserId, onDismiss }: MatchModalProps) {
  ...
  const otherProfile =
    match.user1.userId === currentUserId ? match.user2 : match.user1;
  const myProfile =
    match.user1.userId === currentUserId ? match.user1 : match.user2;
    ...
    <Modal transparent animationType="fade" visible={!!match} testID="match-modal">
      <View style={{ flex: 1, backgroundColor: "rgba(10,10,15,0.95)" }}>
        ...
        >
          {/* Glow rings + photos */}
          <View
            style={{ flexDirection: "row", alignItems: "center", marginBottom: 32, gap: -20 }}
          >
            {/* My photo */}
            <View
              style={{
          ...
          <Text style={{ color: theme.textSecondary, fontSize: 16, marginBottom: 8 }}>
          ...
          >
            <Text style={{ color: theme.success, fontSize: 14, fontWeight: "700" }}>
